    ...
    public:
      ...
      double report(size_t const iteration, size_t const n_iterations, bool const overwrite=true) {
          ...
          if (since > delta) {
              ...
              if (echo > 0) {
                  char tail[16] = "              \n";
                  auto const performance = (total > 0) ? (iteration + 1.)/total : 0;
                  tail[14] = overwrite ? '\r' : '\n'; // with \r only one line is shown in stdout or when following an outfile with tail -f
                  std::printf("# timer started at %s:%d progress %.2f %% of %ld at %g iterations/sec, expect %3g sec more%s",
                               file.c_str(), line, done*100, n_iterations, performance, eta, tail);
                  std::fflush(stdout);
              } // echo
              last_time = now;
          }
