...
namespace pawxml_export {
      ...
      // Export Projector Augmented Wave data in XML format
  {
      ...
      if (std::string("upf") == suffix) { // unified-pseudopotential-format
        ...
        auto const setup_or_dataset = (6 == version) ? "setup" : "dataset";
        std::fprintf(f, "<paw_%s version=\"%.1f\">\n", setup_or_dataset, 0.1*version); // ABINIT: paw_dataset version="0.7", GPAW: paw_setup version="0.6"
    ...
    //  double const prefactor = rg[TRU].rmax/(std::exp(rg[TRU].anisotropy*(rg[TRU].n - 1)) - 1.);
        ...
        if (n_electrons[1] > 0) {
            int constexpr csv = 1; // 1:semicore (not supported by the codes that read PAWXML)
            int const irc = radial_grid::find_grid_index(rg[TRU], r_cut);
            if (0 != spherical_density[TRU](csv,irc) && echo > 0) {
                std::printf("# %s %s Z=%g semicore density must vanish!", Sy, __func__, Z);
            } // launch a warning
        } // has semicore electrons
