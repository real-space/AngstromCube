...
namespace control {
    ...
    *    Variables can be defined in the control.file, the command line or in the code itself.
    *    Variables can be read anywhere in the code while a default value must be given 
    *    for the case of being undefined.
    *    Internally, variables are stored as pairs of two strings, name and value.
    */

    int constexpr default_echo_level = 2;
    int constexpr echo_set_without_warning = -1; // suppress re-definition warnings in control::set
    ...
    status_t read_control_file(char const *filename, int const echo=0); // read variable definitions given in an input file
