...
namespace bitmap {
    ...
    inline status_t test_write_bmp_file(int const echo=0) {
        ...
        for (int rgb = 0; rgb < 3; ++rgb) {
            ...
            for (int y = 0; y < h; ++y) {         auto const y2 = (y - center_y)*(y - center_y);
                for (int x = 0; x < w; ++x) {     auto const x2 = (x - center_x)*(x - center_x);
//                  auto const value = (x2 + y2 < d*d); // step function
    ...
    inline status_t all_tests(int const echo=1) { return test_write_bmp_file(echo); }
