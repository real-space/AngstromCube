...
//
// all views (view2D, view3D and view4D) have:
//   - a default constructor
//   - a wrapping constructor (the memory is provided my a pointer, the leftmost index cannot be checked at runtime)
//   - a memory owning constructor (the memory is allocated with new T[product_of_dims] and free'd in the destructor)
//   - a destructor (which frees the memory if it was allocated)
//   - a move constructor view234D(view234D<T> && rhs);
//   - a move assignment  view234D& operator= (view234D<T> && rhs);
// explicitly deleted:
...
public:
  ...
  view2D(T* const ptr, size_t const stride)
    : _data(ptr), _n0(stride), _n1(DimUnknown), _mem(0) { 
      data_view_debug_printf("# view2D(ptr, stride=%i) constructor\n", stride);
  ...
  view2D(size_t const n1, size_t const stride, T const init_value={0}) 
    : _data(new T[n1*stride]), _n0(stride), _n1(n1), _mem(n1*stride*sizeof(T)) {
      data_view_debug_printf("# view2D(n1=%i, stride=%i [, init_value]) constructor allocates %g kByte\n", n1, stride, _mem*.001);
  ...
  //     data_view_debug_printf("# view2D(view2D<T> const & rhs);\n");
  //     *this = rhs;
  // } // copy constructor
  ...
  //     data_view_debug_printf("# view2D& operator= (view2D<T> const & rhs);\n");
  //     _data = rhs._data;
  //     _n0   = rhs._n0; 
  //     _n1   = rhs._n1;
...
public:
  ...
  view3D() : _data(nullptr), _n0(DimUnknown), _n1(DimUnknown), _n2(DimUnknown), _mem(0) { } // default constructor

  view3D(T* const ptr, size_t const n1, size_t const stride)
    : _data(ptr), _n0(stride), _n1(n1), _n2(DimUnknown), _mem(0) { } // wrapping constructor
  ...
  view3D(size_t const n2, size_t const n1, size_t const stride, T const init_value={0}) 
    : _data(new T[n2*n1*stride]), _n0(stride), _n1(n1), _n2(n2), _mem(n2*n1*stride*sizeof(T)) {
      data_view_debug_printf("# view3D(n2=%i, n1=%i, stride=%i [, init_value]) constructor allocates %g kByte\n", n2, n1, stride, _mem*.001);
  ...
  view3D& operator= (view3D<T> && rhs) {
      data_view_debug_printf("# view3D& operator= (view3D<T> && rhs);\n");
      _data = rhs._data;
      _n0   = rhs._n0;
      _n1   = rhs._n1;
      _n2   = rhs._n2;
  ...
  //     data_view_debug_printf("# view3D& operator= (view3D<T> const & rhs);\n");
  //     _data = rhs._data;
  //     _n0   = rhs._n0;
  //     _n1   = rhs._n1;
  //     _n2   = rhs._n2;
...
public:
  ...
  view4D(T* const ptr, size_t const n2, size_t const n1, size_t const stride) 
    : _data(ptr), _n0(stride), _n1(n1), _n2(n2), _n3(DimUnknown), _mem(0) { } // wrapping constructor
  ...
  view4D(size_t const n3, size_t const n2, size_t const n1, size_t const stride, T const init_value={0}) 
    : _data(new T[n3*n2*n1*stride]), _n0(stride), _n1(n1), _n2(n2), _n3(n3), _mem(n3*n2*n1*stride*sizeof(T)) {
      data_view_debug_printf("# view4D(n3=%i, n2=%i, n1=%i, stride=%i [, init_value]) constructor allocates %g kByte\n", n3, n2, n1, stride, _mem*.001);
  ...
  //     data_view_debug_printf("# view4D(view4D<T> const & rhs);\n");
  //     *this = rhs;
  // } // copy constructor

  // view4D& operator= (view4D<T> && rhs) = delete;
  view4D& operator= (view4D<T> && rhs) {
      data_view_debug_printf("# view4D& operator= (view4D<T> && rhs);\n");
      _data = rhs._data;
      _n0   = rhs._n0;
      _n1   = rhs._n1;
      _n2   = rhs._n2;
  ...
  //     data_view_debug_printf("# view4D& operator= (view4D<T> const & rhs);\n");
  //     _data = rhs._data;
  //     _n0   = rhs._n0;
  //     _n1   = rhs._n1;
  //     _n2   = rhs._n2;
...
#undef    _VIEW2D_HAS_PARENTHESIS
#undef    _VIEW2D_HAS_INDEXING
#undef    _VIEW3D_HAS_PARENTHESIS
#undef    _VIEW3D_HAS_PARENTHESIS_2ARGS
#undef    _VIEW3D_HAS_INDEXING
#undef    _VIEW4D_HAS_PARENTHESIS
#undef    _VIEW4D_HAS_PARENTHESIS_3ARGS
#undef    _VIEW4D_HAS_PARENTHESIS_2ARGS
#undef    _VIEW4D_HAS_INDEXING
