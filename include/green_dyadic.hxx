...
//        nops += flop_count_SHOsum*pow2(R1C2)*pow2(Noco); // collect
  ...
  status_t exchange(                                              // here, nSHO is the global maximum of nsho[ia]
...
//                if (echo > 10) std::printf("# %s idir=%d i4=%d ivec=%i xi^2=%g H1D= ", __func__, i3, i4, i, xi_squared[i3][i4]);
  ...
  inline status_t test_exchange(int echo=0) {
      ...
      for (int Noco = 1; Noco <= 2; ++Noco) {
          ...
          stat += exchange(mat_out, requests, mat_inp, offerings, owner_rank.data(), nall, count, true, MPI_COMM_WORLD, echo);
          delete[] mat_inp;
          delete[] mat_out;
      } // Noco in {1, 2}
      return stat;
