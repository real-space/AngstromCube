...
namespace fermi_distribution {
    ...
    inline status_t test_integration(int const echo=3) {
        double const de = 0.0625, threshold = 1e-15; 
        int const n = 37/de;
        if (echo > 6) std::printf("\n## plot Fermi-Dirac distribution and energy derivative (%d points)\n", n);
        double s0{0}, s1{0};
        ...
        for (int ie = -n; ie < n; ++ie) {
            ...
            if ((ie & 1) && e*e < 16*16 && echo > 6) std::printf("%g %g %g\n", e, f, -dfde);
            s0 += f;
            s1 -= dfde;
        } // e
        if (echo > 6) std::printf("%g %g %g\n", n*de, 0., 0.);
        double const dev[] = {s0/n - 1, s1*de - 1};
        if (echo > 2) std::printf("\n# Fermi-Dirac distribution deviation %.1e %.1e\n", dev[0], dev[1]);
        return (std::abs(dev[0]) > 2*threshold) + (std::abs(dev[1]) > threshold);
    } // test_integration

    inline status_t test_bisection(int const echo=3, int const n=99) {
        std::vector<double> ene(n), wgt(n, 1);
        for (int i = 0; i < n; ++i) {
            ene[i] = simple_math::random(-10., 10.);
        } // i
        double const eF = Fermi_level(nullptr, ene.data(), wgt.data(), n, 3e-2, n*.75, 1, echo);
        density_of_states(n, ene.data(), 3e-2, eF, 3e-4, echo);
        return 0;
    } // test_bisection
    ...
    inline status_t test_FermiLevel_class(int const echo=0) {
        ...
        if (echo > 6) std::printf("# %s eF= %g %s for %g electrons, sum(occupations)= %g\n",
                                __func__, mu.get_Fermi_level()*eV,_eV, n_electrons, sum_occ);
        mu.get_occupations(occ.data(), ene.data(), nb, 1, echo, docc.data());
        return 0;
