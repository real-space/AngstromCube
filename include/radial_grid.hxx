...
// #doc
// Constructor and destructor functions for the radial_grid_t radial grid descriptor radial_grid_t (which is a struct).
//
...
namespace radial_grid {
  ...
  // the recommended number of radial grid points for spherical all-electron atom DFT calculations
  inline int default_points(float const Z_protons=0.f) { return 250*std::sqrt(std::abs(Z_protons) + 9); }

  // find the index that is closest to a given radius
  int find_grid_index(radial_grid_t const & g, double const radius); // declaration only

  // the radial grid prefactor is necessary when the radial grid values are generated on-the-fly
  double get_prefactor(radial_grid_t const & g); // declaration only

  // convert an equation-identifyer char into a human readable const string
