...
// #doc
// The element_config module offers the possibility to auto-configure
// the splitting of core and valence electrons.
// Alternatives are the preconfigured strings in the sigma_config module.
//
...
#include "atom_core.hxx" // ::guess_energy, ::nl_index, ::read_Zeff_from_file
...
namespace element_config {
    ...
    // cheap way to find the chemical symbol: take the first two letters at this pointer
    inline char const* element_symbol(int const iZ) {
        int const z128 = iZ & 127; // == iZ modulo 128
        return &(element_symbols[z128*2]); // this and the following char belong to this element
    } // element_symbol

    // display angular momentum quantum numbers
    char const ellchar[] = "spdfgh+";

    // find a configuration of core and valence electrons for the generation of PAW potentials
    inline sigma_config::element_t get(
...
//   8s
    ...
    inline status_t test_get(int const echo=0) {
        status_t stat(0);
        int const iZ_start = control::get("element_config.Z.start", 1.);
        auto const core_state_localization = control::get("element_config.core.state.localization", -1.); // in units of electrons, -1:inactive
        auto const ionization = control::get("element_config.ionization", 0.); // in units of electrons
        double const rmax = 14.173;
        int cannot_read{0};
        char Sy[4] = "Sy\0";
        for (int iZ = iZ_start; --iZ;) {
            double const Z_core = iZ;
            auto const El = element_symbol(iZ); 
            Sy[0] = El[0]; Sy[1] = El[1];
            auto rg = radial_grid::create_radial_grid(radial_grid::default_points(Z_core), rmax);
            std::vector<double> rV(rg.n, 0.0);
            auto const load_stat = atom_core::read_Zeff_from_file(rV.data(), rg, Z_core, "Zeff", -1., echo, Sy);
            if (0 == load_stat) {
                auto const e = element_config::get(Z_core, ionization, rg, rV.data(),
                                                 Sy, core_state_localization, echo);
                if (echo > 7) { std::printf("# e.Z = %g\n\n", e.Z); }
            } else {
                ++cannot_read;
                if (echo > 3) { std::printf("# failed to read from file \'Zeff.%3.3i\'\n", iZ); }
            }
            radial_grid::destroy_radial_grid(rg);
        } // iZ
        if (cannot_read) { warn("failed to read from %d files \'Zeff.00Z\'", cannot_read); }
        return stat;
    } // test_get

    inline status_t all_tests(int const echo=0) {
        status_t stat(0);
        stat += test_show_all_elements(echo);
        stat += test_get(echo);
        return stat;
    } // all_tests
