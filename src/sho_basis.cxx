...
namespace sho_basis {
  // loads radial basis function that are expanded into a SHO basis from an .xml-file
    ...
    status_t generate_double(
            view2D<double> & matrix // result: on successful exit this is a nSHO(numax) x nbasis matrix
        ...
        if (echo > 11) std::printf("# %s(Z= %g, numax= %d)\n", __func__, Z_core, numax);
        nbasis = 0;
        ...
        if (nullptr == rfset) {
            ...
            matrix = view2D<double>(nsho, nbasis, 0.); // get memory and initialize zero
    ...
    status_t generate(
        ...
        int nbasis{0};
        if (std::is_same<complex_t,double>::value) {
            if (echo > 2) std::printf("# %s<%s> --> generate_double(Z= %g, numax= %d)\n", __func__, complex_name<complex_t>(), Z_core, numax);
            return generate_double(*reinterpret_cast<view2D<double>*>(&matrix), sigma, numax, nbasis, Z_core, echo);
        }
        view2D<double> m;
        auto const load_stat = generate_double(m, sigma, numax, nbasis, Z_core, echo);
        if (0 == load_stat) {
            auto const nsho = sho_tools::nSHO(numax);
            matrix = view2D<complex_t>(nsho, nbasis, complex_t(0)); // get memory and initialize zero        
            for (int isho = 0; isho < nsho; ++isho) {
                for (int j = 0; j < nbasis; ++j) {
                    matrix(isho,j) = m(isho,j); // copy and convert
                } // j
            } // i
        } // stat
        return load_stat;
