...
//       !--------------------------------------
  ...
  status_t test_solver(int const echo=9, int const ng_default=32) {
      ...
      int constexpr I_fd = 0, I_fft = 1, I_rho = 2;
      auto const x = xb[I_fd], x_fft = xb[I_fft], b = xb[I_rho];
      ...
      if (echo > 7) { // get a radial representation through Bessel transform
          ...
          for (int ir = 0; ir < rg.n; ++ir) {
              std::printf("%g %g %g %g %g %g %g\n", rg.r[ir], fr(I_fd,ir)*f, fr(I_fft,ir)*f, fr(I_hartree,ir), fr(I_q2,ir), fr(I_rho,ir)*f, fr(I_rho_radial,ir));
          } // ir
          std::printf("\n\n");
