...
//      if (n_off > 0) printf("# DouglasPeucker eleminate %d points, largest distance^2 is %g\n", n_off, d2max);
  ...
  template<typename real_t>
  std::vector<char> RDP_lossful_compression(real_t const x[], real_t const y[], int const n, float const epsilon=1e-6) {
      std::vector<char> mask(n, 1); // init as true
      RamerDouglasPeucker(mask.data(), x, y, n, 0, epsilon);
      return mask;
