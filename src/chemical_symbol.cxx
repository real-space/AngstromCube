...
#include "chemical_symbol.hxx" // ::SYMBOL_UNKNOWN
...
namespace chemical_symbol {
  ...
  int8_t get(char Sy[4], double const Z, char const blank) {
      ...
      if (iZ != Z) {
          Sy[1 + (' ' != y)] = '*'; // add an asterisk
          // The asterisk marks that the number of nucleons is special, e.g. non-integer
      } // iZ != Z
      return z7;
  } // get
  ...
  // strategy: combine both printable ASCII char S,y into one 16 bit number. For y='\0' --> y=' '
  ...
  int8_t decode(char const S, char const y) {
      switch  (encode_16bit(S, y)) {
          ...
          case encode_16bit('-','1'): return 127; // "-1"
          case encode_16bit('0',' '): return   0; // "0 "
          case encode_16bit('_',' '): return   0; // "_ "
          default:
          {
