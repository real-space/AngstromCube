...
namespace density_generator {
  ...
  void add_to_density_matrices(
      ...
      , complex_t const *const atom_coeff[] // projection coefficents of the wave function // ToDo: make this a plain pointer and work with an offset list
  ...
  status_t atom_coefficients(
        std::vector<data_list<typename grid_operator_t::complex_t>> & atm
      ...
      // ToDo: we need a method to split the pure data and the pointer struct from a data_list
...
// #pragma omp parallel
      for(int ikpoint = 0; ikpoint < nkpoints; ++ikpoint) {
          ...
          for(int iband = 0; iband < nbands; ++iband) {
              ...
              stat += op.get_atom_coeffs(atom_coeff.data(), psi_nk, echo/2); // project
...
//    int const natoms = op.get_natoms();
      std::vector<std::vector<real_t>> scale_factors(natoms);
      std::vector<uint16_t> natom_coeff(natoms);
      for(int ia = 0; ia < natoms; ++ia) {
          int const numax = op.get_numax(ia);
          ...
          for(int iband = 0; iband < nbands; ++iband) {
              ...
              if (occupation[iband] > occ_threshold) {
                  ...
                  add_to_density_matrices(atom_rho, atom_coeff.data(),
                               natom_coeff.data(), natoms, weight_nk, echo, iband, ikpoint);
