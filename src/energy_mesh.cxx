...
namespace energy_mesh {
    ...
    // move these declartions to energy_mesh.hxx if other routines need to make use of these quadratures
    ...
    void show_contour(std::vector<Complex> const & E_points, std::vector<Complex> const & E_weights, int const echo=0) {
        ...
        { // scope: svg file writing
            auto const svg_filename = "energy_contour.svg";
            auto const svg = std::fopen(svg_filename, "w"); // "wx" --> does not overwrite existing file
            if (nullptr != svg) {
                std::fprintf(svg, "<!-- SVG code generated by %s -->\n", __FILE__); // can be viewed at https://editsvgcode.com/
                float constexpr f = 272.10282768626; // display units are 10 pixels per eV
                float constexpr tm = 2; // tick mark length to both sides, 0: none

                float const ext[] = {std::abs(ex[0][0] - ex[0][1]), std::abs(ex[1][0] - ex[1][1])}; // box extent
                auto const b = f*std::max(ext[0], ext[1])/4; // b: half a border in units of pixels
                std::fprintf(svg, "<svg viewBox=\"%g %g %g %g\" xmlns=\"http://www.w3.org/2000/svg\">\n",
                                            f*ex[0][0] - 2*b, f*ex[1][0] - 2*b, f*ext[0] + 4*b, f*ext[1] + 4*b);

                std::fprintf(svg, "  <line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"cyan\" />\n", f*ex[0][0], 0.f, f*ex[0][0], -f*ex[1][1]); // bottom
                std::fprintf(svg, "  <line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"cyan\" stroke-dasharray=\"4\" />\n", 
                                                            f*ex[0][0], -f*ex[1][1], f*ex[0][1] + b, -f*ex[1][1]); // parallel

                std::fprintf(svg, "  <line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"red\" />\n", 0.f, b - f*ex[1][0], 0.f, -f*ex[1][1] - b);
                for (int iy = -.125f*f*ex[1][1]; iy <= 2; ++iy) { // imaginary axis with tick marks
                    if (tm) std::fprintf(svg, "  <line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"red\" />\n", -tm, 10.f*iy, tm, 10.f*iy);
                } // it

                std::fprintf(svg, "  <line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"black\" />\n", f*ex[0][0] - b, 0.f, f*ex[0][1] + b, 0.f);
                for (int ix = .1f*(f*ex[0][0] - b); ix <= .1f*(f*ex[0][1] + b); ++ix) { // real axis with tick marks
                    if (tm) std::fprintf(svg, "  <line x1=\"%g\" y1=\"%g\" x2=\"%g\" y2=\"%g\" stroke=\"black\" />\n", 10.f*ix, -tm, 10.f*ix, tm);
                    if (tm && ix != 0 && 0 == (ix % 10)) std::fprintf(svg, "  <text x=\"%d\" y=\"%d\" style=\"font-size: 3;\">%d eV</text>\n", 10*ix-3, 9, ix);
                } // it

                char const w8colors[4][8] = {"black", "red", "green", "yellow"}; // if circles appear in yellow their weight is neither purely real nor purely imaginary
                for (size_t iE{0}; iE < nE; ++iE) {
                    auto const ep = E_points.at(iE);
                    auto const ew = E_weights.at(iE);
                    float const radius = f*.1f*std::sqrt(std::abs(ew));
                    int const ic = (std::real(ew) != 0)*2 + (std::imag(ew) != 0); // in [0, 3]
                    std::fprintf(svg, "  <ellipse cx=\"%g\" cy=\"%g\" rx=\"%g\" ry=\"%g\" fill=\"white\" stroke=\"%s\" />\n",
                                                std::real(ep)*f, -std::imag(ep)*f, radius, radius, w8colors[ic]);
                } // iE

                std::fprintf(svg, "</svg>\n");
                std::fclose(svg);
                if (echo > 3) std::printf("# file %s (Scalable Vector Graphics) written\n", svg_filename);
            } else {
                if (echo > 2) std::printf("# %s: failed to open file %s for writing, maybe file exists!\n", __func__, svg_filename);
                // warn("# %s: failed to open file %s for writing, maybe file exists!\n", __func__, svg_filename);
            } // nullptr != svg
        } // scope
    ...
    std::vector<Complex> get_energy_mesh(
        ...
        , double   const kBT  // temperature in energy units
        , double   const eBot // contour bottom energy
        , unsigned const nBot // number of points on the bottom path
        , unsigned const nPar // number of points on the parallel path
        , unsigned const nFer // number of points to sample the Fermi-Dirac distribution
        ...
        // nPol == 0 --> Density of States contour, i.e. only nPar is relevant
        // nPol > 0  --> Open Contour with Matsubara poles, use all input parameters
        // nPol < 0  --> Closed Contour without Matsubara poles, nPol is irrelevant, nFer be
        ...
        int jE{0};
        if (0 != nPol) { // scope: sample a path from the real-valued band bottom to a point perpendicular to the real axis
            int const mBot = Gauss_Legendre_quadrature(xi, wi, nBot, echo);
            double const dE = std::abs(nPol)*pikBT;
            assert(dE > 0);
        ...
        if (0 != nPol) { // sample a path parallel to the real axis up to the Fermi level
            int const mPar = Gauss_Legendre_quadrature(xi, wi, nPar, echo);
            double const dE = (E_mu - 30*kBT*(nPol > 0) - eBot)*0.5;
            for (int iE{0}; iE < mPar; ++iE) {
                Ep.push_back(Complex(eBot + (xi[iE] + 1.)*dE, std::abs(nPol)*2*pikBT));
                w8.push_back(Complex(wi[iE]*dE, 0.0)); // real-valued weights
                show_energy_point(Ep[jE], w8[jE], jE, "parallel", echo);
                ++jE;
            } // iE
        } else {
        ...
        if (nPol > 0) { // scope: sample the Fermi-Dirac distribution from the Fermi level upwards in energy parallel to the real axis
            ...
            for (int iE{0}; iE < mFer; ++iE) {
                ...
                w8.push_back(Complex(wi[iE]*dE, 0.0)); // real-valued weights
                show_energy_point(Ep[jE], w8[jE], jE, "fermidirac", echo);
                ++jE;
            } // iE
        } // scope
        ...
        if (nPol >= 0) { // scope: add Matsubara poles
            ...
            auto const nTop = nFer; // close the energy contour at the top similar to "bottom"
            int const mTop = Gauss_Legendre_quadrature(xi, wi, nTop, echo);
            double const dE = std::abs(nPol)*pikBT;
            assert(dE > 0);
...
#undef wi // Fortran indices
    ...
    double abs2(Complex const x) { return pow2(std::real(x)) + pow2(std::imag(x)); }

    status_t test_energy_mesh(int const echo=5) {
        double const eBot = -1, kBT = 1.0080339e-2, beta = 1./kBT; // == 20k Kelvin per Matsubara pole, Fermi energy at zero

        double c_ref{0}; // reference integration result
        int const nEig = control::get("energy_mesh.test.eigenvalues", -1.); // 0: cosine analytically, -1: cosine numerically, >0: random eigenvalues
        char const *fun{"cos"};
        std::vector<double> eigs; // a list of randomly chosen eigenvalues
        if (0 == nEig) {
            fun = "Cos";
            c_ref = -std::sin(-1.); // analytically integrated cosine
        } else { // scope:: compute reference value by integrating over real axis [-1, 0] very densely
            int constexpr m = 1e7; // number of points for numerical reference integration
            auto const dx = 1.0/m; // homogeneous weight for numerical reference integration
            if (nEig > 0) { // create a list of random eigenvalues
                fun = "eig";
                eigs.resize(nEig);
                for (int ieig{0}; ieig < nEig; ++ieig) {
                    eigs.at(ieig) = simple_math::random(0., (ieig + 1)*10*kBT) + 0.5*eBot; // generate random eigenvalues of the Hamiltonian
                } // ieig
                std::sort(eigs.begin(), eigs.end()); // sort ascendingly, mostly for better display
                for (int i{1-m}; i < 2*m; ++i) { // integrate over [-2, 1]
                    auto const x = -(i + .5)*dx; // on real axis
                    for (int ieig{0}; ieig < nEig; ++ieig) {
                        c_ref += 1./((pow2(x - eigs[ieig]) + pow2(kBT))*(std::exp(beta*x) + 1.)); // Lorentzian curve times Fermi-Dirac function
                    } // ieig
                } // i
                c_ref *= kBT/constants::pi; // prefactors of the Lorentzian
                if (echo > 3) {
                    std::printf("# %s with %d random eigenvalues:", __func__, nEig);
                    for (int ieig{0}; ieig < nEig; ++ieig) {
                        std::printf(" %.6f", eigs[ieig]);
                    } // ieig
                    std::printf(" Ha, numerical reference= %g\n", c_ref*dx);
                } // echo
            } else {
                fun = "cos";
                for (int i{0}; i < m; ++i) {
                    auto const x = -(i + .5)*dx;
                    c_ref += std::cos(x); // numerically integrate cosine
                } // i
            }
            c_ref *= dx; // integration weight (constant intervals Delta x)
        }
        ...
        double mindev{9e9}; int mn[4] = {0,0,0,0}; // store value and location of the absolute smallest deviation from the reference value

        for (int nm{0}; nm <= std::abs(nPol); ++nm) {                     // .matsubara
            int const mPol = (nPol < 0) ? -nm : nm;
        for (unsigned nb = (0 != mPol); nb <= nBot*(0 != mPol); ++nb) {   // .bottom
        for (unsigned nf = (0 != mPol); nf <= nFer*(0 != mPol); ++nf) {   // .fermidirac
        for (unsigned np{1}; np <= nPar; ++np) {                          // .parallel

            // generate a complex integration contour
            std::vector<Complex> energy_weights;
            auto const energy_mesh = get_energy_mesh(energy_weights, kBT, eBot, nb, np, nf, mPol, echo/4);
            if (echo > 19) std::printf("# energy mesh with %ld points generated\n", energy_mesh.size());
            auto const nE = energy_mesh.size();
            assert(nE == energy_weights.size());

            // integrate a simple but holomorphic function of which the integral over the real axis is known (c_ref)
            Complex c(0);
            for (size_t iE{0}; iE < nE; ++iE) {
                ...
                if (nEig > 0) {
                    Complex ciE(0);
                    for (int ieig{0}; ieig < nEig; ++ieig) {
                        ciE += Complex(0.,1.)/(x - Complex(eigs[ieig], kBT)); // Lorentzian resonance curve
                    } // ieig
                    auto const con = ciE*(1./constants::pi)*wgt;
                    c += con;
            ...
            auto const dev = c.real() - c_ref;
            if (echo > 8) std::printf("# %s(+energy_contour.bottom=%d .parallel=%d .fermidirac=%d .matsubara=%d) Integral[%s]= (%g, %.1e), reference= %g, delta= %.1e\n",
                                         __func__, nb, np, nf, mPol, fun, c.real(), c.imag(), c_ref, c.real() - c_ref);
            if (std::abs(dev) < std::abs(mindev)) {
                mn[0] = nb; mn[1] = np; mn[2] = nf; mn[3] = mPol; // store where the minimum occurred
                mindev = dev;
            }
        ...
        if (nEig > 0 && echo > 7) {
            std::printf("# %s with %d random eigenvalues:", __func__, nEig);
            for (int ieig{0}; ieig < nEig; ++ieig) { std::printf(" %.6f", eigs[ieig]); }
            std::printf(" Ha, numerical reference= %g\n", c_ref);
        } // echo

        if (echo > 2) { 
            std::printf("# %s: best integration[%s] at energy_contour.bottom=%d .parallel=%d .fermidirac=%d .matsubara=%d, dev= %.1e\n",
                          __func__, fun, mn[0], mn[1], mn[2], mn[3], mindev);
        } // echo

        return 0; // ToDo: automatize test to fail when deviations are large
    } // test_energy_mesh
    ...
    status_t test_Gauss_Legendre_quadrature(int const echo=5) {
        ...
        for (int n{0}; n <= 20; ++n) {
            ...
            if (nn == n) {
                // integrate pure powers of x using the quadrature weights
                std::vector<double> s(n, 0.0);
                for (int i{0}; i < n; ++i) { // for each point in the quadrature
                    double xipow{1}; // init (x[i])^k for k == 0
                    for (int k{0}; k < n; ++k) { // serial forward loop
                        s[k] += xipow*w[i];
                        xipow *= x[i]; // prepare (x[i])^k for k := (k+1)
                    } // k
                } // i
                if (echo > 13) { std::printf("# integrals for n=%d ", n); printf_vector(" %.6f", s); }
                // expected result for pure powers: 2 0 2/3 0 2/5 0 2/7 0 2/9 0 2/11 0 ...
                double dev[] = {0, 0}; // deviation for {even, odd}
                for (int k{0}; k < n; ++k) {
    ...
    status_t all_tests(int const echo) {
        int const which = control::get("energy_mesh.select.test", -1.); // -1 == all
        status_t stat(0);
