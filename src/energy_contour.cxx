...
namespace energy_contour {
...
#ifdef    ENERGY_CONTOUR_SVG_EXPORT
        auto const svg = std::fopen("energy_contour.svg", "w");
        if (nullptr != svg) {
            ...
            for (size_t iE{0}; iE < nE; ++iE) {
                auto const ep = E_points.at(iE), ew = E_weights.at(iE);
                float const radius = f*.1f*std::sqrt(std::abs(ew));
                int const ic = (ew.real() != 0)*2 + (ew.imag() != 0); // in [0, 3]
                std::fprintf(svg, "  <ellipse cx=\"%g\" cy=\"%g\" rx=\"%g\" ry=\"%g\" fill=\"white\" stroke=\"%s\" />\n",
                                              ep.real()*f, -ep.imag()*f, radius, radius, w8colors[ic]);
            } // ep

            std::fprintf(svg, "</svg>\n");
            std::fclose(svg);
            if (echo > 3) std::printf("# file energy_contour.svg (Scalable Vector Graphics) written\n");
        } // nullptr != svg
#endif // ENERGY_CONTOUR_SVG_EXPORT
    ...
    std::vector<Complex> get_energy_mesh(
        ...
        if (0 != nPol) { // sample a path parallel to the real axis up to the Fermi level
            ...
            assert(nPar > 0);
            double const dE = (nPar < 2) ? 1 : ((E_mu + 60*kBT - eBot)/(nPar - 1.));
            int const mFer = 30*kBT/dE;
            for (int iE = mFer - nPar; iE < mFer; ++iE) { // nPar points, one will hit E_mu exactly
        ...
        if (nPol >= 0) { // scope: add Matsubara poles
            ...
            // close the energy contour at the Fermi level similar to "bottom"
            auto const nTop = nFer;
