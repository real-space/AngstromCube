...
\begin{figure}
  ...
  %% 	run -t atom_core.
  \begin{minipage}[c]{.990\textwidth}
	\includegraphics[width=\textwidth]{fig/atom_core_LDA} %%
  \end{minipage}\hfill
  \begin{minipage}[c]{.009\textwidth}
  \end{minipage}
...
are $t \approx n\um{images} \cdot N\um{atoms}^2 \cdot \unit[1.32]{ns}$, c.f.~red data points.
%%%% 27*110592^2 atom-atom pairs in 435.866 sec
\\
...
\begin{figure}
  ...
  The crossover of the dashed lines happens around $18$ atoms.
  }
\end{figure}
%
%
Comparing the prefactors in fig.~\ref{fig:geometry-analysis-scaling} 
we can extrapolate a theoretical crossover for this particular case (bulk, cubic cell) around $18$ atoms.
For any larger number, the linear scaling solution is faster.
