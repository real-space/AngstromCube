...
With that and the decay factor $\exp\left({-\frac{r^2}{2\sigma^2}}\right)$
...
\begin{align}
 ...
 1 &= \int \mathrm d r r^2 f_\ell \exp(-\frac{r^2} {2\sigma^2}) r^{2\ell} \\
...
\end{align}
However, we need the (L1-)normalization w.r.t.~$r^\ell$:
\begin{align}
	f^{-1}_{L1} &= \int\limits_0^\infty \mathrm dr r^2 \exp(-\frac{r^2}{2\sigma^2}) r^\ell \\
				&= \sigma^{\ell + 3} \int\limits_0^\infty \mathrm dx x^{\ell + 2} \exp(-x^2/2) = \sigma^{\ell + 3} d(\ell + 1)  \\
\end{align}
where $d(n)$ is simply the double factorial $n!!$ which needs to be multiplied by $\sqrt{\pi/2}$ for odd $n$.
Therefore, we have to introduce another prefactor $f_{L1}/f_{L2}$ which corrects for the assumed L2 normalization 
imposing the desired L1 normalization.
