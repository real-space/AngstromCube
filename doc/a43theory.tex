...
\begin{align*}
  E^a &= T^a\um{core} + \sum_{ij} \rho^a_{ij} \braketop{\phi^a_i}{\hat T}{\phi^a_j} \\
      &+ E\um{xc}[n^a] \\
      &+ \frac{1}{2} \braketop{ n^a }{ \frac{1}{\vec r - \vec r'} }{ n^a } - Z^a \braket{ n^a }{ \frac{1}{r} } 
...
\begin{align*}
  \tilde E^a &= \sum_{ij} \rho^a_{ij} \braketop{\tilde\phi^a_i}{\hat T}{\tilde\phi^a_j} \\
      &+ E\um{xc}[\tilde n^a] \\
      &+ \frac{1}{2} \braketop{ \tilde\varrho^a }{ \frac{1}{\vec r - \vec r'} }{ \tilde\varrho^a } \\
...
\begin{align}
  \hat H &= \hat T + \tilde V\um{eff}(\vec r) + \sum_a \left( H^a_{ij} - \tilde H^a_{ij} \right)\ket{p^a_i}\bra{p^a_j}
\end{align}
...
(eq.~(\ref{eqn:hamiltonian-true-atom-potential-part}) and (\ref{eqn:hamiltonian-smooth-atom-potential-part}))
...
Then the approach of evaluating eq.~(\ref{eqn:hamiltonian-true-atom-potential-part}) and (\ref{eqn:hamiltonian-smooth-atom-potential-part}) numerically does not lead to extra costs.
Note that during the construction of electrostatic potentials inside the augmentations sphere, 
i.e.~$V\um{es}[n^a - Z^a delta]$ and $V\um{es}[\tilde\varrho^a]$, the boundary conditions must match between
the two and also with the electrostatic potential on the 3D grid, $V\um{es}[\tilde\varrho]$.
This is achieved with the projection coefficients $\braket{\hat g^a_{\ell m}}{V\um{es}[\tilde\varrho]}$.
