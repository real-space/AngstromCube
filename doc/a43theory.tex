...
\begin{figure}
	...
	%%% ./a43 -t hermite_polynomial +verbosity=10
...
\begin{figure}
	%%% how to generate this plot:
	%%% make -j && ./a43 -t sho_overlap +verbosity=9 | grep 'test_Hermite_Gauss_overlap' | sed -e 's/# test_Hermite_Gauss_overlap  distance=//g'
  \begin{minipage}[c]{.990\textwidth}
...
%During the \ac{SHO} tranform onto the local potential we need to take care of the normalizing prefactors:
%Typically, L2-normalizations are multiplied to the \ac{SHO} projection coefficients 
...
\begin{figure}
   ...
   %%% make -j && ./a43 -t spherical_atoms +verbosity=11  +single_atom.echo=6 > out
...
%
% Assume we have a cubic simulation cell with edge length k*R, where R=r_vic, and p periodic boundary conditions.
% and atom density rho=N_atoms/(k*R)^3
% Then, each box has volume R^3 and in average N_atoms/k^3 = n_atoms atoms.
% We have to check the distances between each box with 27 neighbor boxes,
% except for some boxes close to isolated BCs.
% 
% The N^2-algorithm needs      3^p * k^6 * n_atoms^2 distance checks
% The N-algorithm needs at most 27 * k^3 * n_atoms^2 distance checks
...
\begin{figure}
  %% how to generate this plot:
  %%    ./a43 -V -t geometry_analysis +geometry.file=Cu320Zr180.xyz
  \begin{minipage}[c]{.990\textwidth}
	\includegraphics[width=\textwidth]{fig/Cu320Zr180_distance_hist} %% .agr .pdf
  \end{minipage}\hfill
  \begin{minipage}[c]{.009\textwidth}
    % we could place the caption here
  \end{minipage}
  \label{fig:geometry-analysis-histograms}
  \caption{
Histograms of Cu$_{320}$Zr$_{180}$, a metallic glass structure.
% # bond table: pair, min dist in Bohr, bond stats, total= 6762 bonds
% #  Cu-Cu    4.121   4.990 +/- 0.343 in [4.121, 6.230]  1886 bonds
% #  Cu-Zr    4.704   5.599 +/- 0.545 in [4.704, 7.250]  1918 bonds
% #  Zr-Zr    5.562   6.464 +/- 0.735 in [5.562, 8.264]  1040 bonds
% # half bond length and coordination numbers with occurrence
% #    2.494 Bohr coordination for Cu   10_11  11_77  12_171  13_59  14_2
% #    3.307 Bohr coordination for Zr   14_3  15_30  16_60  17_63  18_21  19_3
...
% with +geometry_analysis.elongation=1.1
% # bond table: pair, min dist in Bohr, bond stats, total= 6018 bonds
% #  Cu-Cu    4.121   4.916 +/- 0.245 in [4.121, 5.487]  1722 bonds
% #  Cu-Zr    4.704   5.447 +/- 0.336 in [4.704, 6.381]  1710 bonds
% #  Zr-Zr    5.562   6.182 +/- 0.352 in [5.562, 7.269]  876 bonds
% # half bond length and coordination numbers with occurrence
% #    2.494 Bohr coordination for Cu   8_2  9_28  10_109  11_98  12_83
% #    3.307 Bohr coordination for Zr   12_6  13_26  14_62  15_69  16_16  17_1
...
\begin{figure} [ht]
  \centering
  \begin{minipage}[c]{.990\textwidth}
	\includegraphics[width=\textwidth]{fig/N_eigenvalues_of_charge-deficit_operator} %% .agr .pdf
  \end{minipage}\hfill
  \begin{minipage}[c]{.009\textwidth}
  \end{minipage}
...
\begin{figure} [h]
  \centering
  \begin{minipage}[c]{.990\textwidth}
	\includegraphics[width=\textwidth]{fig/potential_generator_generalized_Gaussian} %% .agr .pdf
  \end{minipage}\hfill
  \begin{minipage}[c]{.009\textwidth}
  \end{minipage}
...
\begin{figure} [ht]
  \centering
  \begin{minipage}[c]{.990\textwidth}
  	% git checkout d85c93e002ae3ad5e611335382b25a899e543c73
    % test/charge_deficit.test.sh
  	% test/data.charge_deficit/
	\includegraphics[width=.75\textwidth]{fig/charge_deficit_Cu_s} %% .agr .pdf
	\includegraphics[width=.75\textwidth]{fig/charge_deficit_Cu_p} %% .agr .pdf
	\includegraphics[width=.75\textwidth]{fig/charge_deficit_Cu_d} %% .agr .pdf
  \end{minipage}\hfill
  \begin{minipage}[c]{.009\textwidth}
  \end{minipage}
...
%The observed behavior of the eigenvalues of the charge deficit operator 
%does not seem to depend on the particular way of how the partial waves were generated.
%Even with a classical generation scheme (\ttt{+single\_atom.partial.wave.method=c}) the instability can be seen.
%%% To be checked again
...
Find the true partial waves $\varphi_{n}(r)$ for the energy parameters $\varepsilon_{n}$.\\
Pseudize $\varphi_{0}(r) \longrightarrow \tilde \varphi_{0}(r)$ with a $4$-coefficient even polynomial times $r^\ell$. Then\\
\begin{align}
	...
	\intertext{and for the higher partial wave}
	\varphi_i &= \braket{SHO_i}{\tilde \varphi_0},
	p_i = \braket{SHO_i}{\tilde p_0} \\
	\tilde p'_1(r) &= \sum_i SHO_i(r) \left( p_i - \varphi_i\sum_j p_j \varphi_j \cdot \left( \sum_k \varphi_k^2 \right)^{-1} \right) \\
	...
	\qquad \text{Gram-Schmidt} \\
\end{align}
The second projector is chosen orthogonal to the lowest smooth partial wave. As a start guess, we use the coefficients
of the lowest projector in the radial SHO basis.
Mind that the SHO basis is suitable for this operation, however, it is in general not capable of representing the smooth partial waves with a modest SHO basis size.
See Fig.~\ref{fig:charge-deficit-copper-recreate-second} for the example of copper with \ttt{4s* 4p* 3d*}.
The eigenvalues of the charge deficit operator are stable (above -1.0) for split energies between $80$ mHartree and $3.35$ Hartree.
The typical split energy used in GPAW PAW setups is $1.0$ Hartree.

\begin{figure} [h]
  \centering
  \begin{minipage}[c]{.990\textwidth}
  	% git checkout ec0d048fe8edddce03b1e619ca9e37576562a16d
    % cd a43/test/data.charge_deficit
    % ../charge_deficit.test.sh > method_r.out
  	% ./charge_deficit.evals.sh method_r.out > method_r.dat
	\includegraphics[width=.99\textwidth]{fig/charge_deficit_method_recreate_second} %% .agr .pdf
  \end{minipage}\hfill
  \begin{minipage}[c]{.009\textwidth}
  \end{minipage}
  \caption{Lower (lower subplot) and upper (upper subplot) charge deficit eigenvalues of Cu with the \ttt{recreate\_second} partial wave generation scheme. The eigenvalues are shown as a function of the energy split between lowest and excited partial wave (in Hartree) for $\ell \in \{s, p, d\}$.} \label{fig:charge-deficit-copper-recreate-second}
\end{figure}

\newpage

\section*{Module green\_dyadic}

Use \ttt{-t green\_dyadic} to test this module:\\
Options are \ttt{+green\_dyadic.test.nb} for $n_B$. $n_B^3$ blocks are used in the \ac{SHO} projection.
\ttt{+green\_dyadic.test.rc} determines a cutoff radius in units of the SHO spread \ttt{+green\_dyadic.test.sigma}
beyond which projection and addition is zero.

\begin{figure} [h]
  \centering
  \begin{minipage}[c]{.990\textwidth}
    \includegraphics[width=.99\textwidth]{fig/green_dyadic-orthogonality_normalization.pdf} %% .agr .pdf
  \end{minipage}\hfill
  \begin{minipage}[c]{.009\textwidth}
  \end{minipage}
  \caption{Error in orthogonality and normalization of the \ttt{green\_dyadic} module. 
           Using \ttt{float} or \ttt{double}, an orthogonality error of only $10^{-6}$ or $10^{-15}$ can be reached, respectively.}
  \label{fig:green-dyadic-orthogonality-normalization}
\end{figure}
