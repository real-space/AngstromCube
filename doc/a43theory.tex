...
\end{align}
Due to the non-linearity of $V\um{xc}$ the evaluation of atom-centered potential matrix elements
...
If, however, we let the core state relax, the core density changes
and we can construct a new true reference potential from which we generate new partial waves.
Then the approach of evaluating the terms~(\ref{eqn:hamiltonian-true-atom-potential-part}) and (\ref{eqn:hamiltonian-smooth-atom-potential-part}) numerically does not lead to extra costs.
...
This is achieved comparing the projection coefficients $\braket{\hat g^a_{\ell m}}{V\um{es}[\tilde\varrho]}$.

The \emph{intelligent zero} inserted to make the local potential smoother inside the augmentation sphere
leads to the term $\bar v^a$. In fact, the electrostatic potential will feature smooth but deep potential well
at the position of the nucleus. $\bar v^a$ is chosen to fill the well.
Different from other implementations of \ac{paw} we added only the contribution of the valence density
with $\bar V$ to the total energy and subtracted it in the atom corrections.
The reason behind this is that there will be tails of the core densities
of neighboring atoms that give contributions which are not cancelled in the atom-centered treatment.
A similar error might appear from the \ac{xc} contribution.
