...
\ttt{geometry\_analysis.num.bins}. Default is $184$ bins with about $30$ bins per \AA{}.
...
%489:      float const elongation = control::get("geometry_analysis.elongation", 1.25); // a bond elongated by 25% over his default length is still counted
%492:      double const max_range = control::get("geometry_analysis.max.range", 6.5*Angstrom2Bohr); // max. analysis range is about 6 Angstrom
%493:      double const bin_width = control::get("geometry_analysis.bin.width", .02*Angstrom2Bohr);
%505:      int const MaxBP        = control::get("geometry_analysis.max.bond.partners", 24.); // max# of bond partners for detailed analysis, 0:inactive
%506:      index_t max_natoms_BP  = control::get("geometry_analysis.max.atoms.with.partners", 2000.);
%575://        auto const unit_name = control::get("geometry_analysis.half.bond.unit", "Bohr");
%584:              half_bond_length[is] = control::get(keyword, default_half_bond);
%846:              int const nhist = control::get("geometry_analysis.num.bins", 184.);
%850:              int const sbond = control::get("geometry_analysis.show.bond.structure", 1.);
%1028:              bool const show_unavailable_pairs = (control::get("geometry_analysis.show.all.pairs", 1.) > 0);
%1064:      auto const geo_file = control::get("geometry.file", "atoms.xyz");
%1079:      int const nspecies = std::min(std::max(2, int(control::get("geometry_analysis.test.nspecies", 128.))), 128);
%1087:      auto const filename = control::get("geometry_analysis.test.file", "species_test.xyz");
%1108:      int const t = control::get("geometry_analysis.select.test", -2.); // -1:all, -2:all but the first
