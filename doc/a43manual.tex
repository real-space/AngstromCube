...
\ttt{input}, \ttt{function} or \ttt{experiments}, depending on what to run.
...
\section{Appendix}
%
\subsection{Example output for atom\_core} \label{sec:example-output-for-atom-core}
%
\small
\begin{verbatim}
...
Furthermore, there is a phantom occupation number \ttt{2e-99} on the $4p$ orbital.
This number is too small to influence \ttt{atom\_core} results but becomes relevant during \ac{PAW} generation.

\newpage

\subsection{Example output for single\_atom} \label{sec:example-output-for-single-atom}
%
\small
...
\subsection{Example output for geometry\_analysis} \label{sec:example-output-for-geometry-analysis}
%
\small
\begin{verbatim}
...
\newpage
\begin{landscape}
\subsection{Example output for radial\_eigensolver} \label{sec:example-output-for-radial-_eigensolver}
%
\small
\begin{verbatim}
...
\begin{figure}[t]
	\centering
	\includegraphics[width=.99\textwidth]{fig/radial_eigensolver_Z100_state_diagram}
...
\subsection{Example output for } \label{sec:example-output-for-3}
%
\small
\begin{verbatim}
\end{verbatim}
