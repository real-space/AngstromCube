...
which couples to the command line interface with a leading \{+}. For example use
\begin{verbatim}
./a43 +verbosity=1 -t
\end{verbatim}
to change the \ttt{verbosity} to minimal when running all unit tests.

\todo[inline]{Ensure that all modules have some meaningful unit tests or at least say that they don't}
