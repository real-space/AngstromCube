...
The iterative solving method \emph{tfQMR} requires to hold eight fields of the size of $X$ in memory. Let's assume $10$ fields to make up for unaccounted overheads.
As $X$ represents the block-sparse Green function, the main \ac{GPU} memory consumption per grid point ($mem\um{total}$) for a large simulation volume $V$ hence depends mostly on the truncation radius $R\um{trunc}$ in units of the grid spacing $h$
\begin{equation} 
	mem\um{total} = 8 \cdot \min\{V\um{cell}, \nicefrac{4}{3} \pi R^3\um{trunc} \}/h^3 \cdot \unit[16]{Byte} \label{eqn:memory}
\end{equation}
assuming double precision complex numbers.
For small simulation volumes ($V\um{cell} \ll R\um{trunc}^3$) the total memory consumption is quadratic in the number of grid points. In the transition region between small and large volumes the curve depends on the chose boundary conditions, c.f.~Fig.~\ref{fig:memory-consumption}.
...
\begin{figure}[t]
	...
	When the truncation sphere (radius of $21.2$ grid spacings) fits into the cubic simulation volume, the transition from the cubic volume-dependence to a constant depends on the boundary conditions.
	In the linear-scaling regime, we can expect a memory requirement for about $110$ real numbers per grid point in the volume and per grid point inside the truncation sphere.}
\end{figure}
...
other advantages related to the on-the-fly evaluation of \ac{PAW} projectors and in the register-data-reusing implementation of the finite-difference kinetic energy might be weakened, hindering performance on the other end.
...
\begin{figure}[t]
	...
	A simple model for the block inside should show a $R^3$-dependence, for the partial hits $R^2$. 
	The memory (solid pink line) is given in MByte for 64 right hand side and \ttt{double} precision complex numbers. 
	The memory overhead compared to the analytical truncation sphere volume is shown in blue.} 
\end{figure}
