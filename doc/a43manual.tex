...
We may add blanks after \ttt{=} but not before it.
\\
...
\todo[inline]{Simplify single\_atom by removing the auto config and then continue the description here}
...
\hline
  \ttt{geometry.file} & \ttt{atoms.xyz}   & Cell parameters and Atomic coordinates \\
  \ttt{basis}         & \ttt{grid}        & Basis set, \ttt{planewaves}, \ttt{sho} \\ 
  \ttt{grid.spacing}  & \unit[0.125]{\AA} & Spacing of the coarse real-space grid \\
  ...
  \ttt{electrostatic.solver} & \ttt{fft}  & Coulomb solver, \ttt{multi-grid}, \ttt{none} \\
  ...
  \ttt{store.waves}   & -                 & Filename to store wave functions \\
\hline
\end{tabular}
\end{table}
%
