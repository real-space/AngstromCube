...
\acrodef{BC}[BC]{boundary condition}
...
when it comes to multi-precision algorithms via templates
and its good interoperability with common \ac{GPU} programming models, mainly \ttt{CUDA}.
...
%
Please try to use the \ttt{CMake} support.
For development purposes, two \ttt{makefile}s are contained in the folder \ttt{src/} (\ttt{src/Makefile} and \ttt{src/Makefile.green}) that work with current compilers, e.g.~\ttt{GNU c++ 7.4.0} or \ttt{clang 14.0.0}.

\noindent
The application \codename{}, while being written in \ttt{C++}, strongly relies on plane-old-data types,
i.e. ~ classes without constructors or destructors as known from C-\ttt{struct}s.
...
\end{verbatim}
It should tell you that the tests ended with \ttt{status= 0}. 
If you want a more verbose output increase the verbosity level, e.g.~by 4
\begin{verbatim}
./a43 --test complex_tools -V
\end{verbatim}
...
which couples to the command line interface with a leading \ttt{+}.
For example use
\begin{verbatim}
...
\begin{verbatim}
./a43 +output.length.unit=Ang +output.energy.unit=eV -t none
\end{verbatim}
...
\end{verbatim}
to verify this.
A shorthand for the lengthy command \ttt{+output.length.unit=Ang} is \ttt{-u=Ang} and similarly,
\ttt{-U=eV} for the display of energies. It changes the default values of the variables \ttt{output.length.unit} and \ttt{output.energy.unit} and can be overwritten later. See
\begin{verbatim}
./a43 -u=Ang -U=eV ...
\end{verbatim}
%
...
\begin{verbatim}
... +logder.step=1e-3 +logder.unit=Ry
\end{verbatim}
which means that the energy spacing for checking of the logarithmic derivatives is \unit[$10^{-3}$]{Rydberg}.
In this situation the same input unit is assumed for \ttt{+logder.start} and \ttt{+logder.stop}.
...
Some quantities are displayed in characteristic units, e.g., plane wave cutoff energies are typically shown also in \ttt{Rydberg} and temperatures are often converted to \ttt{Kelvin}.
...
Let's say we want aluminium in our system, then run:
\begin{verbatim}
./a43 --test single_atom +single_atom.test.Z=13
\end{verbatim}
See Section \ref{sec:single-atom} for more details on the \ttt{single\_atom} module.
% \todo[inline]{Currently, aluminium produces a critical charge deficit operator eigenvalue} %%% Seems solved
%
\noindent
...
where $00Z$ is a 3-digit integer atomic number, so \ttt{./pot/Zeff.013} in our case.
%% \todo[inline]{make the path to pot/ configurable} %% done with +atom_core.path=pot
You can even add \ttt{+single\_atom.init.scf.maxit=33} to let the atomic density relax for $33$ iterations
if e.g.~ the \ac{XC} functional differs from the one that was used to generate the \ttt{Zeff.}-file using the \ttt{atom\_core} module.
...
To track the progress, \ttt{+verbosity=5} is recommended.
...
\begin{verbatim}
./a43 +geometry.file=molecule.xyz +scf.maxiter=1 +control.show=6
\end{verbatim}
%% \todo[inline]{we have not yet defined a functionality of main! only tests so far. Maybe we will never} %% Done, the main functionality of ./a43 without -t will run self_consistency::SCF and ./green will run parallel_potential::SCF
%
We recommend to add \ttt{+control.show=6} for a summary of all variable values used during the calculation. It is a useful guide to write input files.
%
\subsection{Writing an input file} \label{sec:control-file}
%
At some point the command line becomes full.
We can pass more environment variables to \codename{}'s \ttt{control.file}, e.g.:
\begin{verbatim}
./a43 -f control.sh --test geometry_input
\end{verbatim}
...
%
Lines in a \ttt{control.file} are either 
\begin{itemize}
    \item empty
...
\begin{verbatim}
./a43 -t . +control.show=6 +a_new_variable=6.02214e23
\end{verbatim}
...
\hline
     ...
     \ttt{line \#}   & Line number in the control file \\
\hline
\end{tabular}
\end{table}

\subsubsection{Result reproducability} \label{sec:reproducability}
%
There are three \emph{variables} defined that foster reproducability by identifying the code version:
\begin{itemize}
    \item \ttt{version.main} defines the \ttt{git} hash for \ttt{./a43}
    \item \ttt{version.green} for the executable \ttt{./green} and
    \item \ttt{version.atom} for the shared library \ttt{libliveatom.so}     
\end{itemize}
...
\section{Shifted Cartesian Unit Cells} \label{sec:shifted-BC}

When using finite-difference stencils on uniform real-space grids
the simplest case is to define a rectangular unit cell,
i.e.~ all basis vectors are orthogonal.
Then, the finite-difference stencils are sparse and higher-order can easily be applied.
This, however, does not allow a treatment of a minimal number of atoms per unit cell.
For many highly-symmetric crystals computing with a single atom per unit cell is advantageous.
Although it is possible to define a proper non-rectangular unit cell and corresponding
derivative operators in real-space representation, we decided to try a different approach.
We allow \ttt{shifted} \ac{BC}s.

\subsection{What are shifted boundary conditions?}
%
Shifted \ac{BC}s in \codename{} (currently only \ttt{./a43}) are an experimental feature
to enable any Bravais lattice as a unit cell. The \ttt{geometry.file} could be
% #cell 4 4 2 periodic periodic shifted 0 2 2  --> bcc
\begin{verbatim}
2
#cell 2.46 2.13 8.0 periodic shifted periodic 1.23
C 0 0 0
C 1.23 1.065 0
\end{verbatim}
which indicates a graphene layer with two carbons forming a honeycomb structure.
The underlying lattice 2D structure is trigonal and both carbons are $sp_2$-coordinated, i.e.~three bonds in angles of $120\degree$.
The full syntax is
\begin{verbatim}
#cell Lxx Lyy Lzz xBC yBC zBC Lxy [, Lxz, Lyz]
\end{verbatim}
where \ttt{Lxy} indicates in units of \AA{}ngtr\"{o}m 
how much the periodic image in positive $y$-direction of a rectangular box
is shifted in $x$-direction if the $y$-\ac{BC} is \ttt{shifted}.
Additionally, we can define the $z$-\ac{BC} to be \ttt{shifted} as well.
Then, we can specify nonzero \ttt{Lxz} and \ttt{Lyz} in a similar fashion.
With these three additional parameters even trigonal lattices can be mimicked.
Mind: the $x$-direction can never be shifted.

The rectangular box is described with a uniform Cartesian real-space grid
and standard finite-difference operations can be executed.
Only when a stencil crosses a shifted boundary, the code takes care 
to read from the right grid point and apply certain phase factors.
Using interpolation at the boundary, the shift values could take any value.
However, interpolation has not been implemented, so the shift values will
be adopted to match an integer multiple of the grid spacing.
Only positive shifts are allowed, so make sure the unit cell is defined such that the angles between unit vectors are all $\leq 90\degree$.

\subsection{Common cases}
%
While the simple-cubic (SC) lattice can perfectly be simulated with a single atom per Cartesian unit cell,
this is not the case for the body-centered cubic (BCC) and the face-centered cubic (FCC) structures.
However, the shifted boundary makes this possible nevertheless.
%
\subsubsection{Body-centered cubic}
For BCC, the cubic unit cell contains two equivalent atoms.
We slice the cube in $z$-direction exactly in the center, then with \ttt{atoms.xyz}:
\begin{verbatim}
1
#cell 2.86 2.86 1.43 periodic periodic shifted 0 1.43 1.43
Fe 0 0 0
\end{verbatim}
we find that iron has coordination number eight with an interatomic distance of $2.4768\,$\AA{} and typical BCC bond angles,
$70.5\degree_{12\times}$, $109.5\degree_{12\times}$ and $180\degree_{4\times}$
displayed as \ttt{2.48\_8 |  70\_12 110\_12 180\_4}.

%
\subsubsection{Face-centered cubic}
Similar to the BCC example, aluminium FCC needs the \ttt{Lxy}-shift and the \ttt{Lxz}-shift:
\begin{verbatim}
#cell 4.04 2.02 2.02 periodic shifted shifted 2.02 2.02 0
\end{verbatim}
% Al   2.86_12 |  60_24 90_12 120_24 180_6
produces coordination 12 with typical FCC bond angles
$60\degree_{24\times}$, $90\degree_{12\times}$, $120\degree_{24\times}$, $180\degree_{6\times}$ displayed as
\ttt{2.86\_12 |  60\_24 90\_12 120\_24 180\_6}.

%
\subsubsection{Hexagonal close-packed}
The HCP structure requires two atoms per unit cell but only the \ttt{Lxy}-shift:
\begin{verbatim}
2
#cell 2.8752 2.49 4.6952 periodic shifted periodic 1.4376
Au 0 0 0
Au 1.4376 0.83 2.3476
\end{verbatim}
will display \ttt{2.88\_12 |  60\_24 90\_12 110\_3 120\_18 146\_6 180\_3}.\\
Mind that to compute the energy difference between FCC and HCP structures it is, however,
recommended to use the geometries with 12 atoms per Cartesian unit cell
as this makes sure that convergence w.r.t.~grid spacings is the same
(c.f.~\ttt{geometry\_analysis::test\_fcc\_hcp\_files}).
...
\end{verbatim}
will create the file, parse it and analyze its chain-like geometry.
...
%
This part of the \codename{} manual describes various modules in detail.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.99\textwidth]{fig/20240912_logarithmic_derivative_Cu}
	%% generated:
	%% ./a43 -t single_atom +verbosity=8 +logder.step=1e-3 +output.energy.unit=eV
...
by CECAM's ESL\cite{CECAM_Electronic_Structure_Library} \todo[inline]{cite https://esl.cecam.org/data/paw-xml/}
...
If this cannot be imported by the other software,\\
try \ttt{+single\_atom.radial.grid.equation=reciprocal} for GPAW's native radial grid.
...
We recommend to store them in a control file, see Section \ref{sec:control-file}.
%
\begin{table}[h]
...
\end{verbatim}
in the control file or via the command line as
\begin{verbatim}
./a43 --test sigma_config +sigma_config.test.Z=38 \
        +element_Sr="4s 2 5s 2 4p* 6 4d | 2.37 sigma .666"
\end{verbatim}
...
%\subsection{PAW data testing} \label{sec:paw-data-testing}
%The scattering properties of \ac{PAW} data sets should be tested using both, the logarithmic derivative and the radial eigenstate analysis. Both methods are include in the module \ttt{scattering\_test}.
%\subsubsection{Logarithmic derivative} \label{sec:logarithmic-derivative}
%\subsubsection{Radial eigenstate analysis} \label{sec:radial-eigenstate-analysis}
...
\begin{figure}[t]
	\centering
	%% /a43 -t energy_contour +energy_contour.select.test=2 +energy_contour.matsubara=3
	%% git checkout 51e168d41522d3cc0ba87dbdb3b07796038d0db9
...
the central module \ttt{green\_action} making use of the header-only library \ttt{tfQMRgpu} \cite{Baumeister2025}
...
\begin{figure}[t]
	\centering
	%% generated from test/memory_consumption.test.sh --> memory_consumption.dat
	%% git checkout 51e168d41522d3cc0ba87dbdb3b07796038d0db9
	\includegraphics[width=.99\textwidth]{fig/memory_consumption}
...
\subsection{Discussion of the $4 \times 4 \times 4$ Cube structure}
%
From the cube structure of $4 \times 4 \times 4$ grid points there is some overhead to the memory consumption model in Eq.~(\ref{eqn:memory}). Overheads mainly stem from the description of the truncation sphere in terms of cubes.
For small truncation radii these can be severe, however, the effect becomes weaker for large $R\um{trunc}$.
For example, the memory overhead factors for truncation radii $\{10.6, 21.2, 31.8, 42.3\}$ grid spacings are ${2.94, 1.64, 1.47, 1.37}$, respectively. So in the smallest case we need almost $3\times$ as much memory: $19$ cubes are fully inside the truncation sphere while $208$ cubes need to be stored due to partial hits. A partial hit occurs when any of the $64$ source grid points is closer to any of the $64$ target grid points than the truncation radius, but not all of them.
...
\begin{figure}[t]
	\centering
	%% generated from test/memory_consumption.test.sh
	%% git checkout 51e168d41522d3cc0ba87dbdb3b07796038d0db9
	\includegraphics[width=.99\textwidth]{fig/memory_consumption_overhead}
	\label{fig:memory-consumption-overhead}
	\caption{Number of cubes fully inside (black) and number of cubes with partial hits (red) as functions of the truncation radius.
	A simple model for the number of cubes inside should show a $R^3$-dependence, for the partial hits $R^2$ - proportional to the truncation spheres surface area. 
